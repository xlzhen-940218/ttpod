...
public final class MediaStorage {
    ...
    public static String buildOnlineFavGroupID() {
        if (EnvironmentUtils.AppConfig.getTestMode() && EnvironmentUtils.UUIDConfig.getTid() <= 0) {
            throw new IllegalStateException("you must login first");
        }
        return GROUP_ID_ONLINE_FAV_PREFIX + EnvironmentUtils.UUIDConfig.getTid();
    }
    ...
    public static String buildMusicCircleFavGroupIDPrefix() {
        return GROUP_ID_MUSICCIRCLE_PREFIX + EnvironmentUtils.UUIDConfig.getTid() + "_";
    }
